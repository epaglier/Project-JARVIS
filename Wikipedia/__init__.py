...
class WikipediaSkill(MycroftSkill):
    ...
    def initialize(self):
        ...
        intent = IntentBuilder("WikipediaAllIntent").require(
            "WikipediaAllKeyword").require("ArticleTitle").build()
        self.register_intent(intent, self.handle_all_intent)
    ...
    def handle_intent(self, message):
        try:
            ...
            self.speak(summary)
	    all = "Would you like to hear everything about " + title
	    self.speak(all);
    ...
    def handle_all_intent(self, message):
        try:
            title = message.data.get("ArticleTitle")
            self.__feedback_search(title)
            results = wiki.search(title, self.max_results)
